    ...
    struct gptoss_metal_library* library_out)
{
...
#ifdef EMBEDDED_METALLIB_HEADER
    // Try the embedded header first
    autorelease_pool = [[NSAutoreleasePool alloc] init];
    NSError* error_obj = nil;
    library_obj = EMBEDDED_METALLIB_NAMESPACE::createLibrary(device_obj, &error_obj);
    if (library_obj == nil && error_obj != nil) {
        GPTOSS_LOG_ERROR("failed to create Metal library from embedded header: %s",
                         [[error_obj localizedDescription] UTF8String]);
    }
#endif

    if (library_obj == nil) {
        // Try the Mach-O section path (__METAL,__shaders)
        ...
        if (library_data != NULL) {
            ...
            NSError* error_obj2 = nil;
            id<MTLLibrary> lib2 = [device_obj newLibraryWithData:library_blob error:&error_obj2];
            if (lib2 == nil) {
                GPTOSS_LOG_ERROR("failed to create Metal library from Mach-O section: %s",
                                 [[error_obj2 localizedDescription] UTF8String]);
    ...
    if (library_obj == nil) {
        // Final fallback: default library from bundle (usually fails for Python extensions)
        id<MTLLibrary> lib3 = [device_obj newDefaultLibrary];
        if (lib3 == nil) {
...
enum gptoss_status gptoss_metal_buffer_wrap_mtlbuffer(
    const struct gptoss_metal_device* device,
...
{
    (void) device;
    id<MTLBuffer> buffer_obj = __builtin_bit_cast(id<MTLBuffer>, mtl_buffer);
    if (buffer_obj == nil) {
        GPTOSS_LOG_ERROR("failed to wrap existing Metal buffer");
        return gptoss_status_invalid_argument;
    }
    const NSUInteger buffer_length = [buffer_obj length];
    if (byte_offset > buffer_length || size > buffer_length || byte_offset + size > buffer_length) {
        GPTOSS_LOG_ERROR("wrap_mtlbuffer size %zu with offset %zu exceeds buffer length %lu",
            size, byte_offset, (unsigned long) buffer_length);
        return gptoss_status_invalid_argument;
    }
    [buffer_obj retain];
    uint8_t* contents = (uint8_t*) [buffer_obj contents];
    buffer_out->object = (void*) buffer_obj;
    buffer_out->size = size;
    buffer_out->ptr = contents != NULL ? (void*) (contents + byte_offset) : NULL;
    return gptoss_status_success;
}
...
enum gptoss_status gptoss_metal_command_queue_wrap_existing(
    void* command_queue,
    struct gptoss_metal_command_queue* command_queue_out)
{
    if (command_queue == NULL || command_queue_out == NULL) {
        return gptoss_status_invalid_argument;
    }
    id<MTLCommandQueue> command_queue_obj = (id<MTLCommandQueue>) command_queue;
    command_queue_out->object = (void*) command_queue_obj;
    return gptoss_status_success;
}
...
enum gptoss_status gptoss_metal_command_buffer_wrap_existing(
    void* command_buffer,
    struct gptoss_metal_command_buffer* command_buffer_out)
{
    if (command_buffer == NULL || command_buffer_out == NULL) {
        return gptoss_status_invalid_argument;
    }
    id<MTLCommandBuffer> command_buffer_obj = (id<MTLCommandBuffer>) command_buffer;
    command_buffer_out->object = (void*) command_buffer_obj;
    return gptoss_status_success;
}
